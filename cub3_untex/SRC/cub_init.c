...
*/
/*
Oyuncunun görüş alanını (main->fov) 60 derece olarak ayarlar.
Pencere genişliğine (WIN_W) eşit sayıda ışın (main->n_rays) kullanılacağını belirtir.
İlk ışının açısını (main->angle) hesaplar. Bu, oyuncunun bakış açısının (main->pos.vctr.a) görüş alanının yarısı kadar sola kaydırılmasıyla elde edilir.
main->raycast adlı bir dizi oluşturarak, her bir ışın için bellekte yer ayırır. Bu dizi, t_ray türünde yapılarla dolu olacaktır.
Bir döngü kullanarak, her bir ışını başlatır ve onları main->raycast dizisine ekler:
init_ray(main, &main->raycast[i], main->angle) fonksiyonu, i. ışının başlangıç bilgilerini ayarlar.
Ardından, main->angle değeri, her bir ışın için eşit açı aralıklarıyla arttırılır: (main->fov / (double)main->n_rays)
*/
...
void	init_cub(t_cub_main *main)
{
	...
	//oluşturulan görüntü hakkında bilgi verir ve kullanıcının daha sonra değiştirmesine izin verir.
